...
\begin{abstract}
Historical death registers record causes of death in vernacular, misspelled and compound terms (``quinsy'', ``died of grief'', ``congenital chest mischief''). Coding these strings into a modern disease classification is still largely manual work. We ask a narrow question: on chapter-level coding of such strings, does a language model beat a plain fuzzy lookup against a coding dictionary, and if so, where? We use the 3,306 English historic strings of the ICD10h scheme (CC BY 4.0), derive the ICD-10 chapter from each code, hold out 300 strings and use the remaining 3,006 as a dictionary. Before any model was run we fixed the metric, the baselines, a success margin and a kill criterion. A nearest-neighbour lookup by character trigram cosine reaches 0.707 accuracy. Claude Sonnet 5, prompted with only the string and the list of chapters, reaches 0.667 (95\% CI 0.61--0.72); a local Qwen2.5-14B reaches 0.560. The preregistered test therefore fails. The label list we fixed in advance contains one chapter, Injury, that ICD10h never uses as a primary code and that the dictionary therefore cannot emit; the frontier model put 15 strings there. Folding Injury into External cause, the only correction the asymmetry licenses, gives 0.713 against the dictionary's 0.707 (exact McNemar $p=0.92$): a tie, and still short of the 0.757 the kill criterion demanded. The exploratory picture is different. On the 33 held-out strings with no close dictionary neighbour (trigram similarity below 0.5) the dictionary drops to 0.364 while the frontier model holds at 0.758 (paired difference $+0.39$, bootstrap CI 0.21--0.58, exact McNemar $p=0.001$). By macro-F1 the frontier model is also ahead (0.709 vs 0.635). Error analysis shows that most of the frontier model's remaining errors are convention errors rather than medical errors: it medicalises symptom strings that ICD10h keeps under Ill-defined, and it files infectious gut disease under Digestive. A similarity-routed combination (dictionary when a close neighbour exists, model otherwise) reaches 0.750 at the preregistered cut and 0.780 at the best post hoc threshold. We release code, cached outputs and derived data. A preregistered memorisation probe returned empty outputs for all items and is reported as inconclusive.
\end{abstract}
...
\begin{enumerate}
  ...
  \item Measured results for a frontier model (Claude Sonnet 5) and a local open-weight model (Qwen2.5-14B) under a minimal prompt with no dictionary access (Section~\ref{sec:results}). On the preregistered endpoint the frontier model loses to the dictionary; once a label the dictionary can never emit is folded away, the two tie, and the kill criterion still fails.
...
\paragraph{Labels.} From each ICD10h code we derive the ICD-10 chapter from the code's letter and two-digit block: A--B Infectious, C and D00--D48 Neoplasm, D50--D89 Blood, E Endocrine/metabolic, F Mental, G Nervous, H00--H59 Eye, H60--H95 Ear, I Circulatory, J Respiratory, K Digestive, L Skin, M Musculoskeletal, N Genitourinary, O Pregnancy, P Perinatal, Q Congenital, R Ill-defined, S--T Injury, V--Y External cause, Z Factors, U Special. Twenty chapters occur in the data; 18 occur in the held-out set. Two of the 22 names have no support anywhere in the dictionary: Special (U) and Injury (S--T). The ICD10h strings file carries a separate \texttt{ICD10hInjury} column for the nature-of-injury code, so the primary code for a traumatic death is its external cause and no ICD10h string is ever filed under S--T.

This creates an asymmetry we did not see when we fixed the label list, and which we state here rather than defend. The dictionary baseline returns the chapter of a dictionary entry, so it can never emit Injury or Special. The models are offered both names and can. Neither model ever chose Special. Injury was chosen 15 times by the frontier model and 31 times by the local model, always for a string whose gold is External cause. A label that only one side of a comparison can get wrong is not a neutral part of a preregistered protocol; it is a handicap on the model arms, and the preregistration argument (``the list was fixed before the data were seen'') does not repair it. We therefore report every headline number twice: under the label list as preregistered, because the kill criterion was set on it, and under the label list with Injury folded into External cause, which is the only correction the asymmetry licenses. Where the two disagree we say so.
...
Table~\ref{tab:main} gives the headline numbers. \Bfuzzy{} reaches 0.707 (CI 0.653--0.757). \Mcomm{} reaches 0.667 (CI 0.610--0.720) under the preregistered labels, a difference of $-0.040$ against a required $+0.05$. With Injury folded into External cause it reaches 0.713 (CI 0.660--0.763), a difference of $+0.007$. The preregistered verdict is \textsc{killed} under either scoring. \Mlocal{} reaches 0.560, or 0.657 folded, and is below the dictionary on every cut except the HARD tail.

\begin{table}[t]
\centering
\caption{Chapter-level results on the 300 held-out ICD10h strings. HARD: best char-trigram similarity to any dictionary string below 0.5. Bootstrap 95\% CIs in brackets. ``folded'' rows score the model with Injury mapped to External cause (Section~\ref{sec:data}); the dictionary never emits Injury, so its rows are unchanged. Bold marks the best in each column.}
\label{tab:main}
\footnotesize
...
\Mcomm{} & 0.667 [0.613, 0.720] & \textbf{0.758} [0.606, 0.879] & 0.655 [0.603, 0.712] & 0.709 [0.630, 0.768] \\
\midrule
\Mlocal{} folded & 0.657 [0.600, 0.710] & \textbf{0.758} & 0.644 & 0.613 \\
\Mcomm{} folded & \textbf{0.713} [0.660, 0.763] & \textbf{0.758} & 0.708 & \textbf{0.722} \\
\bottomrule
\end{tabular}
\end{table}

The full-set difference between \Mcomm{} and \Bfuzzy{} is not distinguishable from zero under either scoring: paired bootstrap CI $[-0.11, +0.03]$, exact McNemar $p=0.30$ (63 strings the dictionary gets right and the model wrong, 51 the other way) as preregistered; $[-0.06, +0.07]$, $p=0.92$ (54 versus 52) folded. In raw counts the dictionary is right on 212 of 300, the model on 200 as preregistered and 214 folded. The 12-string gap in the preregistered ordering is smaller than the 15-string Injury artefact that produced it. So the honest statement is that a fuzzy lookup and a frontier model with no dictionary access tie on aggregate accuracy, that the preregistered ordering in the dictionary's favour is an artefact of a label the dictionary cannot use, and that the model does not clear the margin we demanded under either reading.

\subsection{The split by dictionary similarity}
\label{sec:split}

The aggregate hides two opposite regimes (Figure~\ref{fig:bins}). On the 267 non-HARD strings, which have a dictionary neighbour at similarity 0.5 or above, \Bfuzzy{} is ahead of \Mcomm{} by $0.094$ as preregistered (CI $[-0.169, -0.022]$ for model minus dictionary, McNemar $p=0.015$; 62 vs 37 discordant) and by $0.041$ folded (CI $[-0.112, +0.030]$, $p=0.29$; 51 vs 40). The non-HARD advantage of the dictionary is therefore real in direction and not established in size: most of it was the Injury label. On the 33 HARD strings the direction reverses and the effect is large under both scorings, since the artefact does not touch them: \Mcomm{} is ahead by $0.394$ (CI $[0.212, 0.576]$, McNemar $p=0.001$; 14 vs 1 discordant). The dictionary collapses to 0.364 there because its nearest neighbour is lexically misleading: ``quinsy'' matches ``insanity'', ``synovitis'' matches ``otitis'', ``beaten with iron bar'' matches ``lithiasis''. The model has no such problem; it knows what quinsy is.
...
Together these three cells account for 34 of the frontier model's 100 preregistered errors, and the other two account for 20 of its 86 folded errors. The dictionary's errors, by contrast, are lexical accidents spread thinly across many cells, the largest being Genitourinary strings matched to Ill-defined neighbours.

\subsection{A similarity-routed hybrid}
\label{sec:routing}

The two instruments fail in different places, which suggests routing on the one quantity we have for free: the best dictionary similarity. Return the dictionary's chapter if that similarity is at least $\tau$, otherwise the model's. Figure~\ref{fig:hybrid} traces accuracy over $\tau$. At the preregistered cut $\tau=0.5$, which involves no tuning on the test set, the hybrid with \Mcomm{} reaches 0.750 (macro-F1 0.689), above both components and just short of the 0.757 target; folding Injury changes nothing here, because the model answered Injury on none of the 33 HARD strings. The best post hoc threshold, $\tau=0.65$, gives 0.780, and the curve is above 0.75 for every $\tau$ between 0.5 and 0.8. With \Mlocal{} as the fallback the hybrid peaks at 0.730 at $\tau=0.5$. An oracle that takes either component's answer whenever one is right reaches 0.877, so roughly half the remaining headroom between the routed hybrid and the oracle is in strings where the two disagree and the similarity does not tell which to trust.
...
\paragraph{The dictionary is the right baseline, and the frontier model only ties it.} A character-trigram lookup with no training reaches 0.707 on this task. Anyone proposing a model for historical cause-of-death coding should have to beat that, on the same split, before claiming anything. The frontier model does not: 0.667 as preregistered, 0.713 once the label it was uniquely able to get wrong is folded away, against a target of 0.757. Most of the held-out strings are near-duplicates of strings the scheme's authors already coded, and on those a model that has not seen the scheme is guessing conventions that the dictionary simply copies.

\paragraph{A preregistered protocol can carry an asymmetry.} We fixed the label list before the data and treated that as protection. It was not. A label that one arm cannot emit is a handicap on the other arms whether or not it was chosen in advance, and the preregistration argument does not answer that. The right response is the one we take in Table~\ref{tab:main}: keep the preregistered scoring, because the kill criterion was set on it, and report the corrected scoring beside it, flagging every conclusion that moves. Here one moved (the full-set ordering) and the ones that matter did not (the kill verdict, the HARD tail, the routed hybrid).
...
\paragraph{Most of the model's deficit is not a knowledge gap.} Three convention errors account for a third of the frontier model's preregistered mistakes. One of them, Injury versus External cause, is the asymmetry above and is removed by one line of post-processing; the other two, Ill-defined and infectious gut disease, would need one sentence in the prompt each. We did not do that here, because the point was to measure the untouched instrument. A model given the ICD10h manual, or a handful of coded examples per chapter, would plausibly close most of this gap. That is a different experiment, and a cheap one.
